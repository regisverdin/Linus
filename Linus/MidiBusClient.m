...
+ (MidiBusClient*) startWithApp:(NSString*)name andDelegate:(id)delegate
{
    ...
    if (!singleton)
    {
        ...
        eMidiBusStatus status = midibus_initialise([name UTF8String],
           ...
           MIDIBUS_TRANSPORT_COREMIDI|MIDIBUS_TRANSPORT_BTMIDI,
...
#if TARGET_OS_IPHONE
        // start omac (update serially, as if running from swift can cause problems)
        // OMACTrace = MBCTrace;
        singleton->omac = [OMACApplicationRegistry instanceWithAppName:name];
        [singleton->omac update];
		[singleton OMACUpdateComplete:eOMACUpdated];
#endif
